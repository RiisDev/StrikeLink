...
namespace StrikeLink.Services.WebService
{
	internal sealed class SqliteReader : IDisposable
	{
		...
		internal static SqliteReader Open(string filePath) => new(File.ReadAllBytes(filePath));
		...
		private static int ReadBigEndianU16(byte[] data, int offset) => (data[offset] << 8) | data[offset + 1];
